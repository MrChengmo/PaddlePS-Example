...
class Main(object):
    ...
    def run_infer(self, model_path, model_name):
        [inference_program, feed_target_names, fetch_targets] = (
            fluid.io.load_inference_model(dirname=model_path, executor=self.exe))
        ...
        for batch_id, data in enumerate(self.reader()):
            ...
            if batch_id % print_step == 0:
                ...
                for var_idx, var_name in enumerate(results):
                    metrics_string += "Infer res: {}, ".format(
                        results[var_idx])
                logger.info("Model: {}, Batch: {}, {}".format(model_name, batch_id,
                                                              metrics_string))

    def init_reader(self):
        self.reader, self.file_list = reader.get_infer_reader(
    ...
    def reset_auc(self):
        auc_var_name = ["_generated_var_0", "_generated_var_1",
